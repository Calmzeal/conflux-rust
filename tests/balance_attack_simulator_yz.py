...
class Parameters:
    ...
    def __repr__(self):
        return f"latency:{self.latency} num_nodes:{self.num_nodes} out_degree:{self.out_degree} withold:{self.withold}"
    ...
    def setup_network(self):
        ...
        for i in range(self.env.num_nodes):
            for j in range(len(self.neighbors[i])):
                peer = self.neighbors[i][j]
                latency = self.neighbor_latencies[i][j]
                if not i in self.neighbors[peer]:
...
def slave_simulator(env):
    return round(Simulator(env, {"withhold": env.withold, "recent_timeout": env.latency, "extra_send": 0}).main(),2)
