...
class RlpIter:
    ...
    def __next__(self):
        ...
        if length > 0:
            try:
                (prefix, type, length, end) = rlp.codec.consume_length_prefix(
...
class ConfluxEthReplayTest(ConfluxTestFramework):
    ...
    def set_test_params(self):
        ...
        self.conf_parameters = {"log_level": "\"debug\"",
                                ...
                                "egress_queue_capacity": "1024", "test_mode":"true"}
    ...
    def run_test(self):
        ...
        for node in self.nodes:
            block_gen_thread = BlockGenThread(
                node, self.log, random.random(), 1.0 / self.num_nodes)
        ...
        for encoded in RlpIter(f):
            ...
            if int(elapsed_time - last_log_elapsed_time) >= 1:
                ...
                self.log.info("elapsed time %s, tx_count %s",
                              elapsed_time, tx_count)
            if speed_diff >= 1:
